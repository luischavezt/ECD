...
function calcularEstadisticas() {
    let stats = {};
    let golesPorGrupo = {}; // Objeto para goles por grupo
    let golesTotales = 0;
    ...
    for (let i = 0; i < todosLosInputs.length; i += 2) {
        ...
        let v1 = parseInt(in1.value) || 0;
        let v2 = parseInt(in2.value) || 0;
        let sumaPartido = v1 + v2;
        golesTotales += sumaPartido;

        // A. Sumar Goles por Grupo
        let contenedorGrupo = in1.closest('[id^="grupo"]');
        if (contenedorGrupo) {
            let letra = contenedorGrupo.id.replace('grupo', '');
            if (golesPorGrupo.hasOwnProperty(letra)) {
                golesPorGrupo[letra] += sumaPartido;
            }
        }

        // B. Lógica de equipos (v, e, g, gr)
        ...
        if (equipos.length >= 2) {
            ...
            if (!stats[e1]) stats[e1] = { v: 0, e: 0, g: 0, gr: 0 };
            if (!stats[e2]) stats[e2] = { v: 0, e: 0, g: 0, gr: 0 };
            
            stats[e1].g += v1; stats[e1].gr += v2;
            stats[e2].g += v2; stats[e2].gr += v1;
            
            if (v1 > v2) { stats[e1].v++; } 
            else if (v2 > v1) { stats[e2].v++; } 
            else { stats[e1].e++; stats[e2].e++; }
        }
    }

    // 3. Actualizar el DOM
    // Actualizar Goles por Grupo (A-L)
    ...
    // 4. Lógica de estadísticas generales
    const nombres = Object.keys(stats);
    if (nombres.length > 0) {
        const getTop = (crit, tipo) => {
            let vals = nombres.map(n => stats[n][crit]);
            let target = tipo === 'max' ? Math.max(...vals) : Math.min(...vals);
            let lista = nombres.filter(n => stats[n][crit] === target);
            return { nombres: lista.join(', '), val: target };
        };

        const update = (idName, idVal, data) => {
            const nameEl = document.getElementById(idName);
            const valEl = document.getElementById(idVal);
            if(nameEl) nameEl.innerText = data.nombres;
            if(valEl) valEl.innerText = data.val;
        };

        update('stat-wins-name', 'stat-wins-val', getTop('v', 'max'));
        update('stat-wins-min-name', 'stat-wins-min-val', getTop('v', 'min'));
        update('stat-draws-name', 'stat-draws-val', getTop('e', 'max'));
        update('stat-goals-name', 'stat-goals-val', getTop('g', 'max'));
        update('stat-goals-min-name', 'stat-goals-min-val', getTop('g', 'min'));
        update('stat-goals-rec-name', 'stat-goals-rec-val', getTop('gr', 'max'));
        update('stat-goals-rec-min-name', 'stat-goals-rec-min-val', getTop('gr', 'min'));
    }
}
